...
@pytest.mark.asyncio
async def test_trade_cooldown(monkeypatch):
    bot = TradingBotMulti()
    bot.client = DummyClient()
    bot.sim = DummySim()
    bot._calc_qty = lambda sym, price: Decimal('0.01')
    bot._last_trade['BTCUSDT'] = time.time()
    monkeypatch.setattr(config, 'TRADE_COOLDOWN_SEC', 1.0)
    # time advanced by only 0.3s -> should skip
    monkeypatch.setattr(time, 'time', lambda: bot._last_trade['BTCUSDT'] + 0.3)
    await bot._trade('BTCUSDT', 'buy_spot', Decimal('0.2'))
    assert bot.position['BTCUSDT'] == Decimal()

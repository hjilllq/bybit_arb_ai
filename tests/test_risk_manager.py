...
def test_symbol_relative_drawdown(monkeypatch):
    called = {}
    def fake_kill(pid, sig):
        called['sig'] = sig
    monkeypatch.setattr(os, 'kill', fake_kill)
    monkeypatch.setattr(config, 'MAX_SYMBOL_REL_DRAWDOWN_USD', Decimal('10'))
    bot = DummyBot({'ETHUSDT': Decimal('20')})
    rm = RiskManager(bot, interval=0)
    rm.evaluate()  # set high water at 20
    bot.real['ETHUSDT'] = Decimal('5')
    rm.evaluate()
    assert called['sig'] == signal.SIGTERM

def test_restore_high_water(monkeypatch):
    called = {}
    def fake_kill(pid, sig):
        called['sig'] = sig
    monkeypatch.setattr(os, 'kill', fake_kill)
    monkeypatch.setattr(config, 'MAX_RELATIVE_DRAWDOWN_USD', Decimal('5'))
    monkeypatch.setattr(config, 'MAX_DRAWDOWN_USD', Decimal('1000'))
    bot = DummyBot({'BTCUSDT': Decimal('4')})
    rm = RiskManager(bot, interval=0, high_water=Decimal('10'))
    rm.evaluate()
    assert called['sig'] == signal.SIGTERM

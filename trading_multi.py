...
class TradingBotMulti:
    ...
    def __init__(self, client: APIClient | None = None):
        ...
        self.price: Dict[str, Decimal] = {s: Decimal('0') for s in config.TRADE_PAIRS}
        ...
        self._trade_count: Dict[str, int] = {s: 0 for s in config.TRADE_PAIRS}
        self._day_start = time.time()
        self.risk_manager = None
        self.risk_state = {"high_water": Decimal(), "sym_high_water": {}}
        self._load_state()
    ...
    async def _loop(self, sym: str):
        while True:
            ...
            try:
                ...
                logger.warning("loop %s error: %s", sym, exc)
    ...
    async def _trade(self, sym: str, action: str, edge: Decimal):
        ...
        if self._trade_count[sym] >= config.MAX_DAILY_TRADES:
            logger.debug("Skip trade: %s daily limit", sym)
            return
        async with self._lock[sym]:
            ...
            new_qty = new_qty + (qty if side == "Buy" else -qty)
            exposure = abs(new_qty) * price
            for s, q in self.position.items():
                if s != sym:
                    exposure += abs(q) * (self.price.get(s) or price)
            if exposure >= config.MAX_EXPOSURE_USD:
                logger.debug("Skip trade: total exposure limit")
                return
    ...
    async def close_all_positions(self):
        for sym, qty in list(self.position.items()):
            ...
            await self.client.place_order(sym, side, abs(qty))
            pnl = (price - (self.entry[sym] or price)) * qty
            self.real[sym] += pnl
            self.position[sym] = Decimal()
            self.entry[sym] = None
            self.unreal[sym] = Decimal()
            POSITION_SIZE.labels(sym=sym).set(0.0)

    async def shutdown(self, reason: str):
        logger.warning("Shutdown: %s", reason)
        ALERTS.error(f"Остановка: {reason}")
        await self.close_all_positions()
        await self.close()

    # --- persistence -----------------------------------------------------
    def _maybe_persist(self) -> None:
        if time.time() - self._last_persist > config.STATE_SAVE_SEC:
            self._persist_state()
            self._last_persist = time.time()
    ...
    def _check_day_reset(self) -> None:
        if time.time() - self._day_start > 86400:
            self._day_start = time.time()
            for sym in self._trade_count:
                self._trade_count[sym] = 0
        ...
        save_state(
            {
                ...
                **(
                    {
                        "high_water": getattr(self.risk_manager, "high_water", Decimal()),
                        "sym_high_water": getattr(self.risk_manager, "sym_high_water", {}),
                    }
                ),
            }
        )
        for sym in config.TRADE_PAIRS:
            self.pnl.log(sym, self.real[sym], self.unreal[sym], self.capital.equity)
    ...
    def _load_state(self) -> None:
        ...
        if "high_water" in data:
            self.risk_state["high_water"] = data["high_water"]
        if "sym_high_water" in data:
            self.risk_state["sym_high_water"] = data["sym_high_water"]
